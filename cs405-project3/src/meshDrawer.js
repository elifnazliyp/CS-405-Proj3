...
void main()
{
	...
	float ambient = 0.35;
	float diff = max(dot(normal, lightdir), 0.0); // Lambertian reflection
	float spec = 0.0;
	float phongExp = 8.0;

	/////////////////////////////////////////////////////////////////////////////
	// PLEASE DO NOT CHANGE ANYTHING ABOVE !!!
	// Calculate the specular lighting below.

	vec3 viewDir = normalize(-fragPos); // View direction

	if (diff > 0.0) {
		vec3 reflectDir = reflect(-lightdir, normal); // Reflection direction
		spec = pow(max(dot(reflectDir, viewDir), 0.0), phongExp); // Phong reflection model
	}
